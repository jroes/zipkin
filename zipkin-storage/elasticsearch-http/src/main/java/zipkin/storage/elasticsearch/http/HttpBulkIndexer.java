...
class HttpBulkIndexer<T> {
  ...
  HttpBulkIndexer(ElasticsearchHttpClient delegate, String typeName, JsonAdapter<T> adapter) {
  ...
  void writeDocument(T object) throws IOException {
    adapter.toJson(body, object);
    body.writeByte('\n');
  }

  /** Creates a bulk request when there is more than one object to store */
  public ListenableFuture<Void> execute() { // public to allow interface retrofit
    Call post = delegate.client.newCall(new Request.Builder()
        .url(delegate.baseUrl.resolve("/_bulk"))
    ...
    return new CallbackListenableFuture<Void>(post) {
      @Override Void convert(ResponseBody responseBody) throws IOException {
        if (!indices.isEmpty()) {
          delegate.flush(Joiner.on(',').join(indices));
        }
        return null;
      }
    }.enqueue();
