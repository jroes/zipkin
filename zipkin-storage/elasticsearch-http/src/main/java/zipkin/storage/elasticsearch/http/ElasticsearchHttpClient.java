...
import zipkin.moshi.DependencyLinkAdapter;
import zipkin.moshi.SpanAdapter;
import zipkin.storage.elasticsearch.InternalElasticsearchClient;

import static com.google.common.base.Preconditions.checkArgument;
import static com.google.common.base.Preconditions.checkState;
import static zipkin.moshi.internal.JsonReaders.collectValuesNamed;
import static zipkin.moshi.internal.JsonReaders.enterPath;
 ...
 * conflicts.
 */
final class ElasticsearchHttpClient extends InternalElasticsearchClient {
  ...
  static final class Factory implements InternalElasticsearchClient.Factory {
    ...
    Factory(InternalElasticsearchHttpClientBuilder builder) {
    ...
    @Override public InternalElasticsearchClient create(String allIndices) {
      return new ElasticsearchHttpClient(this, allIndices);
    }
  ...
  final JsonAdapter<Span> spanAdapter = new SpanAdapter();
  final JsonAdapter<DependencyLink> dependencyLinkAdapter = new DependencyLinkAdapter();

  ElasticsearchHttpClient(Factory f, String allIndices) {
  ...
  @Override protected void ensureTemplate(String name, String indexTemplate) throws IOException {
    ...
    Call putTemplate = client.newCall(new Request.Builder()
        .url(templateUrl)
  ...
  void flush(String index) throws IOException {
    ...
    try (Response response = client.newCall(flushRequest).execute()) {
      if (!response.isSuccessful()) {
        throw new IllegalStateException("response failed: " + response);
      }
    }
  ...
  protected ListenableFuture<List<String>> collectBucketKeys(String[] indices,
    ...
    Call searchRequest = client.newCall(new Request.Builder().url(lenientSearch(indices, SPAN))
  ...
  @Override protected ListenableFuture<List<Span>> findSpans(String[] indices, QueryBuilder query) {
    ...
    return new SearchResultFuture(searchRequest, spanAdapter).enqueue();
  }
  ...
  protected ListenableFuture<List<DependencyLink>> findDependencies(String[] indices) {
    ...
    Call searchRequest =
        client.newCall(new Request.Builder().url(lenientSearch(indices, DEPENDENCY_LINK))
            .post(RequestBody.create(APPLICATION_JSON, body))
            .tag("search-dependencyLink").build());

    return new SearchResultFuture(searchRequest, dependencyLinkAdapter).enqueue();
  }
